...
class Tracker:
    ...
    def initiate_track(self, detection):
        ...
        print(f"Frame ?: Initiated track {self.next_id} with confidence {detection.confidence}")
        self.next_id += 1

    def predict(self):
        for track in self.tracks:
            track.predict()
        print(f"Frame ?: Predicted {len(self.tracks)} tracks")

    def camera_update(self):
        if self.cmc is not None:
            warp_matrix = self.cmc.get_warp_matrix()
            print(f"Frame ?: Applying CMC with warp matrix {warp_matrix}")
            for track in self.tracks:
                apply_cmc(track, warp_matrix)
        else:
            print("Frame ?: No CMC applied")
    ...
    def gated_metric(self, tracks, detections, track_indices, detection_indices):
        ...
        print(f"Frame ?: Gated metric - Min cost: {cost_matrix_min}, Max cost: {cost_matrix_max}, Shape: {cost_matrix.shape}")
        return cost_matrix, cost_matrix_min, cost_matrix_max

    def match(self, detections):
        confirmed_tracks = [i for i, t in enumerate(self.tracks) if t.is_confirmed()]
        unconfirmed_tracks = [i for i, t in enumerate(self.tracks) if not t.is_confirmed()]
        print(f"Frame ?: Confirmed tracks: {len(confirmed_tracks)}, Unconfirmed tracks: {len(unconfirmed_tracks)}")
        ...
        print(f"Frame ?: Matches: {len(matches)}, Unmatched tracks: {len(unmatched_tracks)}, Unmatched detections: {len(unmatched_detections)}")
        return matches, unmatched_tracks, unmatched_detections

    def update(self, detections):
        print(f"Frame ?: Received {len(detections)} detections")
        matches, unmatched_tracks, unmatched_detections = self.match(detections)
        for track_idx, detection_idx in matches:
            self.tracks[track_idx].update(detections[detection_idx])
            print(f"Frame ?: Updated track {self.tracks[track_idx].track_id}, confirmed: {self.tracks[track_idx].is_confirmed()}")
        for track_idx in unmatched_tracks:
            self.tracks[track_idx].mark_missed()
            print(f"Frame ?: Marked track {self.tracks[track_idx].track_id} as missed, confirmed: {self.tracks[track_idx].is_confirmed()}")
        ...
        print(f"Frame ?: Active confirmed tracks: {len(active_targets)}")
        features, targets = [], []
        for track in self.tracks:
